...
import PathMotionPlugin from "rc-tween-one/lib/plugin/PathMotionPlugin";
  ...
  getChildrenToRender = () => {
    ...
    return Array(amount)
      ...
      .map((k, i) => {
        ...
        const style = {
          transform: `translate(${
            Math.random() * startArea.width + startArea.x
          }px, ${Math.random() * startArea.height + startArea.y}px)`,
        };
    ...
    return (
      <div className="snow-demo-wrapper">
        <div className="snow-demo">
          {this.state.show && <Snow onEnd={this.onEnd}>{children}</Snow>}
        </div>
      </div>
    );
  }
